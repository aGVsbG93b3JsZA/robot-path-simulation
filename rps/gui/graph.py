...
class Graph:
    ...
    属性:
        graph: np.array, 0和1构成的矩阵, 1表示障碍物, 0表示通路
        start: Point, 路径起始点
        end: Point, 路径目标点
    ...
    def neighbors(self, point:Point):
        ...
        """
        status = 0b0000
        for step in self.ADJ:
            neighbor_point = point + step
            if neighbor_point in self:
                status += self.ADJ[step]
                yield neighbor_point, self.ADJ_DIST
        for st in self.DIAG:
            if status & st:
                neighbor_point = point + self.DIAG[st]
                if neighbor_point in self:
                    yield neighbor_point, self.DIAG_DIST
